...
def get_namespace_name(ds: DesignState, root_node: AddrmapNode, node: Node) -> str:
    """
    Return namespace name of node
    """
    return get_node_prefix(ds, root_node, node).title().replace("_", "") + "RWTest"

def roundup_pow2(x: int) -> int:
    return 1<<(x-1).bit_length()

def get_struct_member_name(node: Node) -> str:
    return kwf(node.inst_name)